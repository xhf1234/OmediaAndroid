 ...
 * 
 */
public class SocialGraphFragment extends BaseFriendFragment {
	private Logger logger = Logger.getLogger(SocialGraphFragment.class);

	private static Pattern ADD_FRIEND_PATTERN = Pattern.compile("^"
			+ UrlConst.BaseUrl + "addfriend=(\\{[^\\}]*\\})$");
	private static Pattern SHOW_FRIEND_PATTERN = Pattern.compile("^"
			+ UrlConst.BaseUrl + "friend=(\\{[^\\}]*\\})$");

	@Override
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
			Bundle savedInstanceState) {
		return inflater.inflate(R.layout.fragment_social_graph, container,
				false);
	}
